...
return_type robotSystem::read(const rclcpp::Time&, const rclcpp::Duration&)
{
  ...
  if (!motor_state)
  {
    RCLCPP_ERROR(rclcpp::get_logger("robotSystem"), "Feedback message from motors wasn't received");
  ...
  for (auto i = 0u; i < motor_state->name.size(); i++)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("robotSystem"), "Position or velocity feedback not found for joint %s",
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("robotSystem"), "Position feedback: %f, velocity feedback: %f",
